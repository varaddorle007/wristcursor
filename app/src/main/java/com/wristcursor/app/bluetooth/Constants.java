...
class Constants {
    ...
    private static final byte[] HIDD_REPORT_DESC = {
        ...
        // Deliberately 8-bit. 16-bit axes would remove the per-report range limit, but hosts cache
        // the HID descriptor from pairing time: macOS kept the original 8-bit layout and then read
        // our 6-byte reports with the old 4-byte map, so the high byte of X landed in Y and the low
        // byte of Y landed in the wheel — moving the wrist vertically scrolled the page. Staying
        // byte-compatible with what every already-paired host expects is worth far more than the
        // extra range, which the burst drain below covers anyway.
        (byte) 0x05, (byte) 0x01, //       Usage page (Generic Desktop)
    ...
    private static final String SDP_NAME = "WristCursor";
    private static final String SDP_DESCRIPTION = "WristCursor HID Device";
    private static final String SDP_PROVIDER = "WristCursor";
    /**
     * Request the smallest latency budget the Android HID stack accepts. The Watch 6 otherwise
     * batches best-effort output for hundreds of milliseconds, even though the app submits each
     * packet on time.
     */
    private static final int HID_REPORT_BYTES = 9;
    private static final int HID_INTERVAL_US = 8_000;
    private static final int HID_TOKEN_RATE = HID_REPORT_BYTES * 1_000_000 / HID_INTERVAL_US;
