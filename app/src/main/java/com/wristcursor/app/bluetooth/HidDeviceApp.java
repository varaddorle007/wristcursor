...
public class HidDeviceApp
    ...
    @Nullable private volatile BluetoothDevice device;
    ...
    @Nullable private volatile BluetoothHidDevice inputHost;
    ...
    boolean registerApp(BluetoothProfile inputHost) {
        ...
        try {
            // The Watch 6 batches default/best-effort HID output. Reserve the tiny regular L2CAP
            // budget required for pointer reports instead of letting a 300+ ms burst form.
            boolean accepted =
                    this.inputHost.registerApp(
                            Constants.SDP_RECORD, null, Constants.QOS_OUT, Runnable::run, callback);
            Log.i(TAG, "registerApp(low-latency QoS) accepted=" + accepted);
            return accepted;
        } catch (SecurityException e) {
            Log.e(TAG, "BLUETOOTH_CONNECT required to register HID app", e);
            registered = false;
            return false;
        }
    }
    ...
    public void sendMouse(boolean left, boolean right, boolean middle, int dX, int dY, int dWheel) {
        ...
        if (inputHost != null && device != null) {
            // A zero report is still needed to release a mouse button. HidDataSender avoids
            // scheduling idle zero reports, so this does not add background Bluetooth traffic.
