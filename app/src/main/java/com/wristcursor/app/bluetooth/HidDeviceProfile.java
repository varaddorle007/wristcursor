...
public class HidDeviceProfile {
     ...
     * @return {@code true} if the stack accepted the request, {@code false} otherwise.
     */
    @MainThread
    boolean connect(BluetoothDevice device) {
        if (service == null) {
            Log.w(TAG, "connect: HID profile proxy not ready");
            return false;
        }
        try {
            // Do not gate bonded connects on UUID filter — some phones/PCs advertise HID
            // UUIDs and were incorrectly rejected.
            boolean accepted = service.connect(checkNotNull(device));
            Log.i(TAG, "BluetoothHidDevice.connect(" + device.getAddress() + ") => " + accepted);
            return accepted;
    ...
    /** @return {@code true} if the HID_DEVICE profile proxy is connected. */
    public boolean isReady() {
        return service != null;
    }

    @Nullable
    BluetoothProfile getService() {
        return service;
    }
    ...
    void disconnect(BluetoothDevice device) {
        if (service != null) {
            service.disconnect(checkNotNull(device));
        }
    }
