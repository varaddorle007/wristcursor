...
public class SettingsUtil {
    ...
    private static final String SETTINGS_PREF = "com.wristcursor.app.SETTINGS";
    ...
    @StringDef({
        ...
        SettingKey.GESTURE_PINCH,
        SettingKey.GESTURE_SHAKE_BACK,
        SettingKey.SHOW_CLICK_BUTTONS
    })
    public @interface SettingKey {
        ...
        String GESTURE_PINCH = "pref_settingGesturePinch";
        String GESTURE_SHAKE_BACK = "pref_settingGestureShakeBack";
        String SHOW_CLICK_BUTTONS = "pref_settingShowClickButtons";
    }
    ...
    private static final Map<String, Boolean> defaults =
            new ImmutableMap.Builder<String, Boolean>()
                    ...
                    // Wear: keep HID session when screen dims / system dialogs appear
                    .put(SettingKey.STAY_CONNECTED, true)
                    .put(SettingKey.GESTURE_PINCH, true)
                    .put(SettingKey.GESTURE_SHAKE_BACK, true)
                    .put(SettingKey.SHOW_CLICK_BUTTONS, false)
                    .build();
    ...
    public @HandMode int getMouseHand() {
        // Zero is HandMode.LEFT, which rotates the pointer axes 90 degrees. Persist CENTER for
        // installations that predate this setting so an upgrade cannot silently select LEFT again.
        if (!sharedPref.contains(SettingKey.MOUSE_HAND)) {
            putMouseHand(HandMode.CENTER);
            return HandMode.CENTER;
        }

        int hand = sharedPref.getInt(SettingKey.MOUSE_HAND, HandMode.CENTER);
        if (hand == HandMode.LEFT || hand == HandMode.CENTER || hand == HandMode.RIGHT) {
            return hand;
        }

        // Recover from a malformed or stale stored value with the unrotated mapping.
        putMouseHand(HandMode.CENTER);
        return HandMode.CENTER;
    }
