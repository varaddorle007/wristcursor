...
public class InputActivity extends WearableActivity {
            ...
            new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    if (ACTION_SCREEN_OFF.equals(intent.getAction())) {
                        // Keep mouse UI alive — do not finish on screen-off.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        wakeLock =
                powerManager.newWakeLock(
                        SCREEN_DIM_WAKE_LOCK | ACQUIRE_CAUSES_WAKEUP, "WristCursor:PokeScreen");

        // Do NOT finish the activity on HID disconnect — that instantly closed the
        // WowMouse screen whenever SDP briefly unregistered.
    ...
    public void onEnterAmbient(Bundle ambientDetails) {
        ...
        // Keep the WowMouse screen — finishing here made the UI vanish instantly
        // whenever Wear briefly entered ambient / AOD.
        if (wakeLock != null && !wakeLock.isHeld()) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK || keyCode == KeyEvent.KEYCODE_STEM_PRIMARY) {
    ...
    private void ensureKeyboardController() {
        if (keyboardController == null) {
    ...
    @Override
    public void finish() {
        Log.e(TAG, "finish() called", new RuntimeException("finish-stack"));
        super.finish();
    }

    private void wakeUpAndFinish() {
        Log.e(TAG, "wakeUpAndFinish()", new RuntimeException("wake-finish-stack"));
